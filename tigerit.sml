...
fun tree s0 =
       let     fun say s = s
               ...
               fun stm(SEQ(a,b),d) =
                       ...
                       sayln(",")^stm(b,d+1)^say(")")
               | stm(LABEL lab, d) = indent(d)^say("LABEL ")^say(tigertemp.labelAstring(lab))
               | stm(JUMP (e,_), d) =  indent(d)^say("JUMP(")^exp(e,d+1)^say(")")
               | stm(CJUMP(r,a,b,t,f),d) = indent(d)^say("CJUMP(")^
                       relop(r)^sayln(",")^
                       exp(a,d+1)^sayln(",")^exp(b,d+1)^sayln(",")^
                       indent(d+1)^say(tigertemp.labelAstring t)^say(",")^say(tigertemp.labelAstring f)^say(")")
               ...
               | exp(TEMP t, d) = indent(d)^say("TEMP ")^say(tigertemp.tempAstring t)
               | exp(ESEQ(s,e),d) = indent(d)^sayln("ESEQ(")^stm(s,d+1)^sayln(",")^
                       exp(e,d+1)^say(")")
               | exp(NAME lab, d) = indent(d)^say("NAME ")^say(tigertemp.labelAstring lab)

...
fun canonizeFrags l =
		let 	
				fun aux (PROC{body, frame})=
						let 
								val res = canonize body
								val _ = fragmentos := !fragmentos @ [CanonProc( res, frame)] 
						in
							(*print("\n\t"^(name frame)^"\n");
							List.app (print o tigerit.tree) res*)() 
						end
					| aux (STRING (etiqueta, cadena)) = fragmentos := !fragmentos @ [CanonString (etiqueta, cadena)]
